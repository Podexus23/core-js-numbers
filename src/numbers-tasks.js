...
function getSumOfDigits(num) {
  return `${num}`.split('').reduce((prev, acc) => +prev + +acc);
}
 ...
 * 0.1, 0.2, 0.3 => 0.6
 */
function getSumOfNumbers(...all) {
  return all.reduce((prev, acc) => prev + acc, 0).toFixed(2);
}
